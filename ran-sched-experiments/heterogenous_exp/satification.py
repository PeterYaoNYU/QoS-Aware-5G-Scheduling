...
def get_cumubytes(fname, n_slices):
    ...
    slice_cumu_bytes = [[] for i in range(n_slices)]
    slice_cumu_rbs = [[] for i in range(n_slices)]
    ...
    for fid in range(n_users):
        ...
        # print("the current fid: ", fid,  " belongs to slice ID: ", sid)
        if sid == -1:
            continue
        slice_cumu_bytes[sid].append( cumu_bytes[fid] )
        slice_cumu_rbs[sid].append( cumu_rbs[fid] )
...
for i in range (10):
    print("i: ", i)
    cumu_rbs, cumu_bytes = get_cumubytes_customized_begin_end( INPUT_DIR + "/cap_" + INTRA + str(0) + ".log", n_slices, 1000 * i, 1000 * (i+1), cumu_rbs, cumu_bytes )
...
plt.title('Satisfication capped maxcell')
# plt.title('Satisfication maxcell')
# plt.title('Satisfication nvs')


plt.xticks(tick_positions, tick_labels)
plt.savefig("satisfication rate_cap" + FTYPE )
