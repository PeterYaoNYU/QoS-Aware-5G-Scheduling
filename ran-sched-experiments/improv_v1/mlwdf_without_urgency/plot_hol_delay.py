...
INPUT_DIR="three_video_mlwdf_mt_15slices"
FTYPE=".png"
...
def get_hol_schemes(dname, slice_begin, slice_end, begin_idx):
    ...
    for i in range(begin_idx, begin_idx+1):
        ...
        all_hol['mix'] = get_hol( dname + "/mix_" + str(i) + ".log", slice_begin, slice_end )
        # print("mt: ", all_hol["mt"])
...
def plot_hol_delay(ofname, slice_begin, slice_end, plot_info, begin_idx):
    ...
    ax.grid( axis="both", alpha=0.2 )
    plt.tight_layout()
    fig.savefig( "./" + INPUT_DIR +"/"+ ofname + plot_info + FTYPE )
...
def plot_hol_delay_compare_15_slices(begin_idx):
    ...
    # Retrieve HOL data for the 'mix' scheme
    mix_x_1, mix_y_1, mix_99_1 = get_cdf(all_hol_1['mix'], 0.0, 30)
    mix_x_2, mix_y_2, mix_99_2 = get_cdf(all_hol_2['mix'], 0.0, 30)
    mix_x_3, mix_y_3, mix_99_3 = get_cdf(all_hol_3['mix'], 0.0, 30)
    ...
    # Plot for Mix
    # ax3.plot(mix_x_1, mix_y_1, "y--", label="slice 0 till 4", color=COLORS[0], linewidth=2.5)
    # ax3.plot(mix_x_2, mix_y_2, "r--", label="slice 5 till 9", color=COLORS[1], linewidth=2.5)
    ax3.plot(mix_x_3, mix_y_3, "b--", label="slice 10 till 14", color=COLORS[2], linewidth=2.5)
    ax3.set_xlabel("Queueing Delay(s)", fontsize=default_font + 4)
    ax3.set_ylabel("Ratio", fontsize=default_font + 4)
    ax3.legend(fontsize=default_font + 2)
    ax3.tick_params(axis="both", labelsize=default_font)
    ax3.grid(axis="both", alpha=0.2)
    ax3.title.set_text("Mix")
    
    plt.tight_layout()
    fig.savefig("mlwdf_vs_mt_vs_mix" + str(begin_idx) + FTYPE)
    
    # Print the 99th percentile delay values
    ...
    print("mix_99_1: %f, mix_99_2: %f, mix_99_3: %f" % (mix_99_1, mix_99_2, mix_99_3))
    print("******************************")
