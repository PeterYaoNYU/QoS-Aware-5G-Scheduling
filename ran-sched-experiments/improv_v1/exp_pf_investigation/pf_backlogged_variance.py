...
def save_fig(index):
    ...
    filename = "./exp-customize-20slices/max_throughput_{}_2nd.log".format(index)  
    ...
    filename = "./exp-customize-20slices/pf_{}_2nd.log".format(index)
    lines = read_log_file_reverse(filename)
    data = extract_data(lines, 5, 9)
    print(min(data.values()), max(data.values()))
    ...
    filename = "./exp-customize-20slices/random_{}_2nd.log".format(index)
    lines = read_log_file_reverse(filename)
    data = extract_data(lines, 5, 9)
    print(min(data.values()), max(data.values()))
    ...
    print("Interslice is mix0 *******************************")
    filename = "./exp-customize-20slices/mix_{}_2nd.log".format(index)
    lines = read_log_file_reverse(filename)
    data = extract_data(lines, 5, 9)
    print(min(data.values()), max(data.values()))
    variances_mix = calculate_variance(data, ues_per_slice, 5, 9)
    print(variances_mix)
    ...
    bar_mix = ax.bar(positions + 2 * bar_width, variances_mix, bar_width, label='Mix')

    # Adding labels and title
    ax.set_xlabel('Slices')
    ax.set_ylabel('Variance of Cumulative Bytes')
    ax.set_title('Variance Comparison Across Different Scheduling Algorithms log {} with random as baseline_2'.format(index))
    ax.set_xticks(positions + bar_width)
    ax.set_xticklabels([f'Slice {i+1}' for i in range(num_slices)])
    ax.legend()

    plt.savefig("pf_backlogged_variance_{}.png".format(index))
