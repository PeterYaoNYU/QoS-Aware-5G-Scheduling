...
void PacketScheduler::StopSchedule() {
  DoStopSchedule();
}

void PacketScheduler::DoStopSchedule() {}

PacketScheduler::FlowToSchedule::FlowToSchedule(RadioBearer* bearer,
                                                int dataToTransmit) {
  m_bearer = bearer;
  m_allocatedBits = 0;
  m_transmittedData = 0;
...
void PacketScheduler::DeleteFlowsToSchedule(void) {
  ClearFlowsToSchedule();
  delete m_flowsToSchedule;
}

PacketScheduler::FlowsToSchedule* PacketScheduler::GetFlowsToSchedule(
    void) const {
...
void PacketScheduler::FlowToSchedule::SetSpectralEfficiency(
    std::vector<double>& s) {
  m_spectralEfficiency = s;
}

std::vector<double> PacketScheduler::FlowToSchedule::GetSpectralEfficiency(
    void) {
  return m_spectralEfficiency;
}

void PacketScheduler::FlowToSchedule::SetWidebandCQI(int cqi) {
  m_wideBandCQI = cqi;
}

int PacketScheduler::FlowToSchedule::GetWidebandCQI() {
...
void PacketScheduler::FlowToSchedule::UpdateAllocatedBits(int allocatedBits) {
  ...
  int transmittedPackets = ceil(availableBytes / 1513.0);
...
int PacketScheduler::FlowToSchedule::GetAllocatedBits(void) const {
  return m_allocatedBits;
}

int PacketScheduler::FlowToSchedule::GetTransmittedData(void) const {
  return m_transmittedData;
}

void PacketScheduler::FlowToSchedule::SetDataToTransmit(int dataToTransmit) {
  m_dataToTransmit = dataToTransmit;
}

int PacketScheduler::FlowToSchedule::GetDataToTransmit(void) const {
...
std::vector<int>* PacketScheduler::FlowToSchedule::GetListOfSelectedMCS() {
  return &m_listOfSelectedMCS;
}

void PacketScheduler::FlowToSchedule::SetCqiFeedbacks(
...
void PacketScheduler::InsertFlowToSchedule(RadioBearer* bearer,
                                           int dataToTransmit,
  ...
  std::cout << "\t  --> selected flow: "
            ...
            << " data:" << dataToTransmit << " sinr:" << wideSINR << std::endl;
#endif

  GetFlowsToSchedule()->push_back(flowToSchedule);
}

void PacketScheduler::UpdateAllocatedBits(FlowToSchedule* scheduledFlow,
                                          int allocatedBits, int allocatedRB,
                                          int selectedMCS) {
...
void PacketScheduler::CheckForDLDropPackets() {
  RrcEntity* rrc =
  ...
  for (std::vector<RadioBearer*>::iterator it = bearers->begin();
    ...
    (*it)->GetMacQueue()->CheckForDropPackets(
        (*it)->GetQoSParameters()->GetMaxDelay(),
        (*it)->GetApplication()->GetApplicationID());
    ...
    if ((*it)->GetRlcEntity()->GetRlcModel() == RlcEntity::AM_RLC_MODE) {
      ...
      amRlc->CheckForDropPackets((*it)->GetQoSParameters()->GetMaxDelay(),
                                 (*it)->GetApplication()->GetApplicationID());
    }
  }
}

void PacketScheduler::UpdateTimeStamp() {
  m_ts += 1;
}

unsigned long PacketScheduler::GetTimeStamp() {
  return m_ts;
}

// dataToTransmit is in unit of bytes
void PacketScheduler::InsertFlowToUser(RadioBearer* bearer, int dataToTransmit,
                                       std::vector<double> specEff,
                                       std::vector<int> cqiFeedbacks) {
  int userID = bearer->GetUserID();
  int bearer_priority = bearer->GetPriority();

  for (auto it = m_usersToSchedule->begin(); it != m_usersToSchedule->end();
       ++it) {
  ...
  user->m_requiredRBs +=
      (dataToTransmit * 8 / amc->GetTBSizeFromMCS(amc->GetMCSFromCQI(wideCQI)));
}

void PacketScheduler::CreateUsersToSchedule(void) {
  m_usersToSchedule = new UsersToSchedule();
}

void PacketScheduler::DeleteUsersToSchedule(void) {
  ClearUsersToSchedule();
  delete m_usersToSchedule;
}

void PacketScheduler::ClearUsersToSchedule() {
  for (auto it = m_usersToSchedule->begin(); it != m_usersToSchedule->end();
       ++it) {
    delete *it;
  }
...
PacketScheduler::UsersToSchedule* PacketScheduler::GetUsersToSchedule(
    void) const {
...
void PacketScheduler::UserToSchedule::SetSpectralEfficiency(
    std::vector<double>& s) {
  m_spectralEfficiency = s;
}

std::vector<double>& PacketScheduler::UserToSchedule::GetSpectralEfficiency(
    void) {
  return m_spectralEfficiency;
}
...
int PacketScheduler::UserToSchedule::GetWidebandCQI() {
  return m_wideBandCQI;
}
...
void PacketScheduler::UserToSchedule::UpdateAllocatedBits(int allocatedBits) {
  m_allocatedBits += allocatedBits;
}

int PacketScheduler::UserToSchedule::GetAllocatedBits() {
  return m_allocatedBits;
}