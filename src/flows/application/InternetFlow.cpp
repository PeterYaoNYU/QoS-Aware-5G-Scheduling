...
int get_avg_flowsize() {
  ...
  for (int i = 0; i < InternetFlow::m_typeflow; i++) {
    if (i == 0)
      ...
      avg_flowsize +=
          (InternetFlow::m_flowcdf[i] - InternetFlow::m_flowcdf[i - 1]) *
          InternetFlow::m_flowsize[i];
  }
  return (int)avg_flowsize;
}

int InternetFlow::m_typeflow = 11;
int InternetFlow::m_flowsize[] = {1460,   2920,    4380,   7300,
                                  10220,  58400,   105120, 200020,
                                  389820, 1733020, 3076220};
double InternetFlow::m_flowcdf[] = {0.5,   0.6,  0.7, 0.75, 0.8, 0.8125,
                                    0.825, 0.85, 0.9, 0.95, 1};
...
int InternetFlow::GetSize(void) const {
