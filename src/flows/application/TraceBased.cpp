...
void TraceBased::DoStart(void) {
  Simulator::Init()->Schedule(0.0, &TraceBased::Send, this);
}

void TraceBased::DoStop(void) {}

void TraceBased::SetTraceFile(std::string traceFile) {
  LoadTrace(traceFile);

#ifdef APPLICATION_DEBUG
  std::cout << " TRACE_BASED_DEBUG: Add file trace to Trace-Based "
               "Application Client.....  DONE"
            << std::endl;
#endif
}

void TraceBased::LoadTrace(std::string traceFile) {
  if (m_entries->size() > 0) {
    m_entries->clear();
  }
...
void TraceBased::LoadDefaultTrace(void) {
  uint32_t time, index, prevTime = 0;
  uint16_t size;
  char frameType;
  TraceEntry entry;
...
void TraceBased::Send(void) {
  ...
  while (true) {
    for (int i = 0; i < (iter->PacketSize) / MAXMTUSIZE; i++) {
      ...
      IPHeader* ip =
          new IPHeader(GetClassifierParameters()->GetSourceID(),
...
void TraceBased::UpdateFrameCounter(void) {
  m_frameCounter++;
}

int TraceBased::GetFrameCounter(void) {
  return m_frameCounter;
}

void TraceBased::PrintTrace(void) {
  std::cout << "Print Trace " << std::endl;
  std::vector<TraceEntry>::iterator it;

  int n = 0;
  for (it = m_entries->begin(); it != m_entries->end(); it++) {
    std::cout << it->FrameIndex << " " << it->FrameType << " " << it->TimeToSend
              << " " << it->PacketSize << std::endl;
    n++;
    if (n == 10)
      return;
  }
